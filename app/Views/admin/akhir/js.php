...
<script type="text/javascript">
    ...
    $(window).ready(function() {
        ...
        $('.formMultiDelete').submit(function(e){
            ...
            if(jmldata.length === 0){
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    html: `<p class="mg-b-10">Anda akan menghapus <b>${jmldata.length} data</b> Laporan Akhir Mahasiswa</p>`,
                    icon: 'warning',
    ...
    function _getAkhir() {
        $('#viewTable').DataTable({
            ...
            "ajax": "<?= site_url('Backend/Akhir/getData') ?>",
        });
    }
    function _btnAdd() {
        $.ajax({
            url: "<?= site_url('Backend/Akhir/form') ?>",
            success: function(response) {
    ...
    function _simpan() {
        ...
        if (jenis == "") {
            $("#jenis + span").addClass("is-invalid");
            $("#jenis + span").focus(function() {
                $(this).addClass("is-invalid");
            });
        } else {
            $('#jenis').removeClass('is-invalid');
            $("#jenis + span").removeClass("is-invalid");
            $("#jenis + span").focus(function() {
                $(this).removeClass("is-invalid");
            });
        }
        if (kelompok_id && user_id && keterangan && jenis) {
          $.ajax({
              url: "<?= site_url('Backend/Akhir/insert_data'); ?>",
              type: "POST",
              data: ajaxData,
              contentType: false,
              cache: false,
              ...
              success: function(response) {
                  if (response == "Sukses") {
                      Toast.fire({
                          icon: "success",
                          title: "Data Laporan Akhir Kelompok Mahasiswa berhasil ditambahkan"
                      });
                      $('.form-data')[0].reset();
    ...
    function _detail(id) {
        $.ajax({
            url  : "<?= site_url('Backend/Akhir/detail') ?>",
            type : "POST",
            data: {
              id: id
            },
